...
class Turtlebot2FlexPlannerDetectorSM(Behavior):
    ...
    def create(self):
        ...
        _sm_container_0 = ConcurrencyContainer(outcomes=['finished', 'failed', 'red_ball', 'other_ball'],
                                               input_keys=['plan'], conditions=[
        ...
        with _sm_container_0:
            ...
            OperatableStateMachine.add('ball_detector',
                                       ...
                                       transitions={'unavailable': 'failed', 'invalid': 'failed', 'red_ball': 'red_ball',
                                                    'green_ball': 'other_ball', 'blue_ball': 'other_ball'},
                                       autonomy={'unavailable': Autonomy.Off, 'invalid': Autonomy.Off,
                                                 'red_ball': Autonomy.Off, 'green_ball': Autonomy.Off,
                                                 'blue_ball': Autonomy.Off},
        ...
        with _state_machine:
            ...
            OperatableStateMachine.add('Container',
                                       _sm_container_0,
                                       transitions={'finished': 'Continue', 'failed': 'Continue', 'red_ball': 'saw_red',
                                                    'other_ball': 'saw_other_ball'},
                                       autonomy={'finished': Autonomy.Inherit, 'failed': Autonomy.Inherit,
                                                 'red_ball': Autonomy.Inherit, 'other_ball': Autonomy.Inherit},
                                       remapping={'plan': 'plan'})

            # x:435 y:146
            OperatableStateMachine.add('Continue',
                                       OperatorDecisionState(outcomes=["yes", "no", "clearcostmap", "patrol"],
                                                             hint="Continue planning to new goal?", suggestion="yes"),
                                       transitions={'yes': 'Receive Goal', 'no': 'finished', 'clearcostmap': 'ClearCostmap',
                                                    'patrol': 'PatrolPath'},
                                       autonomy={'yes': Autonomy.Full, 'no': Autonomy.Full, 'clearcostmap': Autonomy.Full,
                                                 'patrol': Autonomy.Full})
            ...
            OperatableStateMachine.add('spin',
                                       TimedTwistState(target_time=20, velocity=0.01, rotation_rate=0.628,
            ...
            OperatableStateMachine.add('spin_right',
                                       RotateAngleState(target_time=20.0, target_angle=-360.0, cmd_topic='/cmd_vel',
            ...
            OperatableStateMachine.add('AutoReplan',
                                       OperatorDecisionState(outcomes=["yes", "no"],
                                                             hint="Re-plan to current goal?", suggestion="yes"),
